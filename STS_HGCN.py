...
class STS_HGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        m = self.rhythmAtt_A(torch.cat((self.m1.weight,
                                        ...
                                        self.m4.weight,
                                        self.m5.weight), 0))
        imp = m.unsqueeze(2).expand_as(torch.zeros(5, 5, self.dim)).contiguous().view(5, -1, 1).unsqueeze(3).expand_as(
            torch.zeros(5, 5 * self.dim, 1, self.chan_num))
        x = self.ELU(self.bn1(
            sum(torch.einsum('pijk,bijk->bpijk', (imp, x)).split(self.dim, 2)).view(s1, s2, s3, s4) + self.rhythmAtt_G1(
                x)))
        G1 = self.resGCN1(x, x, L).contiguous()
        G1 = self.ELU(self.bn2(sum(torch.einsum('pijk,bijk->bpijk', (imp, G1)).split(self.dim, 2)).view(s1, s2, s3,
                                                                                                        s4) + self.rhythmAtt_G2(
            G1)))
        G2 = self.resGCN2(G1, torch.add(x, G1), L).contiguous()
        G2 = self.ELU(self.bn3(sum(torch.einsum('pijk,bijk->bpijk', (imp, G2)).split(self.dim, 2)).view(s1, s2, s3,
                                                                                                        s4) + self.rhythmAtt_G3(
            G2)))
        G3 = self.resGCN3(G2, torch.add(torch.add(x, G1), G2), L).contiguous()
        G3 = self.ELU(self.bn4(sum(torch.einsum('pijk,bijk->bpijk', (imp, G3)).split(self.dim, 2)).view(s1, s2, s3,
                                                                                                        s4) + self.rhythmAtt_G4(
            G3)))
        G4 = self.resGCN4(G3, torch.add(torch.add(torch.add(x, G1), G2), G3), L).contiguous()
        G4 = self.ELU(self.bn5(sum(torch.einsum('pijk,bijk->bpijk', (imp, G4)).split(self.dim, 2)).view(s1, s2, s3,
                                                                                                        s4) + self.rhythmAtt_G5(
            G4)))
